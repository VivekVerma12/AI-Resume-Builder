import { BriefcaseBusiness, Linkedin, Mail, MapPin, Phone, User } from 'lucide-react'
...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    const fields = [
        ...
        { key: "phone", label: "Phone Number Name", icon: Phone, type: "tel", required: true },
        { key: "location", label: "Location", icon: MapPin, type: "text", required: true },
        { key: "profession", label: "Profession Name", icon: BriefcaseBusiness, type: "text", required: true },
                ...
                <label>
                    {data.image ? (
                        <img src={typeof data.image === 'string' ? data.image : URL.createObjectURL(data.image)}
                            alt='user-images' className='w-16 h-16 rounded-full object-cover mt-5 ring ring-slate-300
                        hover:opacity-80' />
                    ) : (
            ...
            {fields.map((field) => {
                ...
                return (
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        <input type={field.type} value={data[field.key] || ""} onChange={(e) => handleChange(field.key, e.target.value)} className='mt-1 w-full px-3
                        py-2 border border-gray-300 rounded-lg foucs:ring focus:ring-blue-500 focus:border-blue-500 outline-none
