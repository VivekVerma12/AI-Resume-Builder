import { FilePenLine, FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const uploadResume = async (event) => {
    try {
      ...
      const { data } = await api.post('ai/upload-resume', { title, resumeText },
        {
  ...
  const editTitle = async (event) => {
    try {
      event.preventDefault();
      const { data } = await api.put('resumes/update', { resumeId: editResumeId, resumeData: { title } },
        {
          headers: { Authorization: token }
        }
      )
  ...
  return (
    ...
    bg-clip-text text-transparent sm:hidden'> Welcome, Vivek Verma</p>
        ...
        from-green-500 text-white rounded-full'/>
          ...
          cursor-pointer' onClick={() => {
            ...
            <PlusIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br
        from-purple-500 text-white rounded-full'/>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          ...
          cursor-pointer' style={{
                ...
                <FilePenLineIcon className='size-7 text-slate-700' />
                <p className='text-sm group-hover:scale-105 transition-all px-2 text-center' style={{ color: baseColor }} >{resume.title}</p>
                <p className='absolute bottom-1 text-[11px] text-slate-400 group-hover:text-slate-500 transition-all duration-300 px-2 text-center'> Updated on {new Date(resume.updatedAt).toLocaleDateString()}</p>
        ...
        {showCreateResume && (
          <form onSubmit={createResume} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10
          flex items-center justify-center' onClick={() => {
            ...
            w-full max-w-sm p-6'>
              ...
              <button className='w-full py-2 bg-green-600 text-white rounded hover:text-slate-600 '> Create Resume</button>
        ...
        {showUploadResume && (
          <form onSubmit={uploadResume} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10
          flex items-center justify-center' onClick={() => {
              setShowUploadResume(false);
            }}>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg
        ...
        {editResumeId && (
          <form onSubmit={editTitle} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10
          flex items-center justify-center' onClick={() => {
              setEditResumeId('');
            }}>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg
            w-full max-w-sm p-6'>
              ...
              <input onChange={(e) => setTitle(e.target.value)} value={title} type='text' placeholder='Enter Resume Title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required />
              <button className='w-full py-2 bg-green-600 text-white rounded hover:text-slate-600 '> Upload </button>
              <XIcon className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors'
                onClick={() => {
